...
use crate::header::{
    ...
    Header, ImageType, Orientation,
};
use crate::limits::Limits;
use crate::metadata::{
    Cfa, DisplayChannels, DisplayFunction, Keyword, KeywordOrigin, KeywordSet, Property,
    PropertyScope, PropertySet, PropertyType, PropertyValue, Resolution, ResolutionUnit,
};
use crate::normalize::Range;
use crate::samples::SampleFormat;
use crate::xisf::block::{
    Checksum, Codec, Compression, Location, Subblock, check_subblock_sums, parse_checksum,
    parse_compression, parse_encoding, parse_location, parse_subblocks,
};
...
struct Cache {
    ...
    keywords: std::collections::HashMap<usize, (Keyword, bool)>,
    ...
    /// written down.
    chains: std::collections::HashMap<ChainKey, Keyword>,
    properties: std::collections::HashMap<(usize, PropertyScope), Option<Property>>,
    cfa: std::collections::HashMap<usize, Option<Cfa>>,
    resolution: std::collections::HashMap<usize, Resolution>,
    display_function: std::collections::HashMap<usize, DisplayFunction>,
}
...
fn read_geometry(attr: Option<&str>) -> (Option<Geometry>, Option<DeclineReason>) {
...
fn read_color_space(attr: Option<&str>) -> (Option<ColorSpace>, Option<DeclineReason>) {
    match attr {
...
fn read_sample_format(attr: Option<&str>) -> (Option<SampleFormat>, Option<DeclineReason>) {
    match attr {
        None => (
            None,
            Some(malformed(
...
fn read_byte_order(attr: Option<&str>) -> (bool, Option<DeclineReason>) {
    match attr {
...
fn read_pixel_storage(attr: Option<&str>) -> (Option<PixelStorage>, Option<DeclineReason>) {
    match attr {
...
fn read_offset(attr: Option<&str>) -> (f64, Option<DeclineReason>) {
    let Some(text) = attr else {
        return (0.0, None);
    };
...
fn read_checksum(attr: Option<&str>) -> (Option<Checksum>, Option<DeclineReason>) {
...
fn read_bounds(declared: Option<&str>, format: Option<SampleFormat>) -> Bounds {
...
fn read_keyword<'a>(doc: &'a Doc, node: usize, origin: KeywordOrigin) -> Option<RawKeyword<'a>> {
    Some(RawKeyword {
        node,
...
type ChainKey = (KeywordOrigin, Vec<usize>);
...
fn malformed(reason: impl Into<Arc<str>>) -> DeclineReason {
    DeclineReason::new(DeclineClass::Malformed, reason)
}

fn unsupported(reason: impl Into<Arc<str>>) -> DeclineReason {
...
fn decline_from(error: Error) -> DeclineReason {
    match error {
...
mod tests {
    ...
    fn image(image_attrs: &str) -> String {
        format!(
    ...
    fn tiny(image_attrs: &str) -> String {
        image(&format!(
            r#"geometry="4:4:1" sampleFormat="UInt8" {image_attrs}"#
        ))
    }

    fn class(occurrence: &Occurrence) -> Option<DeclineClass> {
